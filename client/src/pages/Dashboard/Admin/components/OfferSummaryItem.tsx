...
export const GAME_AVAILABILITY_STATUS = {
  AVAILABLE: 'available',
  NOT_AVAILABLE: 'not available',
} as const;

export type GameAvailabilityStatus =
  (typeof GAME_AVAILABILITY_STATUS)[keyof typeof GAME_AVAILABILITY_STATUS];
export type Offer = {
  id: string;
  uid: string;
  title: string;
  extendedTitle?: string;
  dynamicFilters: any; // החלף ב-interface המתאים
  sellerId: any; // החלף ב-interface המתאים
  offerName: string;
  date: string;
...
export const OfferSummaryItem = ({ payload }: Props) => {
  // const { isOpen, setIsOpen, setOrderSummary } = usePartnerClaimContext();
  const { name, _id, uid, status, createdAt } = payload;
        ...
        <div className='flex flex-wrap justify-between gap-4'>
          <div className='flex flex-col gap-3'>
            <h2 className='first-letter:uppercase font-bold font-tti-bold text-[clamp(1.35rem,4vw,2rem)] leading-none'>
              {/* {extendedTitle}{' '} */}
              <span className='capitalize text-brand-primary-color-1'>{name}</span>
            </h2>
          ...
          <GradientBordered className='rounded-[.25rem] before:rounded-[.25rem] before:transition-all before:bg-gradient-bordered-light overflow-clip inline-flex w-auto h-auto self-start'>
            <Link
              to={ROUTER_PATH.OFFER_EDIT.replace(ROUTE_PARAM.UID, uid)}
