...
export const OffersSearchBox = () => {
  ...
  // });
  const [searchOffers, { data: offerCurrenciesData }] = useLazyGetGameCurrenciesQuery();
  ...
  const isGameNotFound = offerCurrenciesData?.data?.length === 0 && query !== '';

  const comboBoxChange = async (offerUid: string) => {
    if (!isGameNotFound) {
      setSelectedItem(offerUid);

      navigate(ROUTER_PATH.CURRENCIES_SINGLE.replace(ROUTE_PARAM.UID, offerUid));
    }
  };
          ...
          >
            <Combobox.Options className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-brand-black-80 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
              {isGameNotFound ? (
                ...
                offerCurrenciesData?.data?.map((offer) => (
                  <Combobox.Option
                    key={offer._id}
                    className={({ active }) =>
