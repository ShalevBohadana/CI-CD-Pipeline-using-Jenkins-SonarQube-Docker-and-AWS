...
import { useLazyGetPromosQuery } from '../../../../../redux/features/promo/promoApi';
import { Combobox } from '@headlessui/react';
import { Transition } from '@headlessui/react';

export const PromosSearchBox = () => {
  ...
  // });
  const [searchOffers, { data: offerPromosData }] = useLazyGetPromosQuery();
  ...
  const isGameNotFound = offerPromosData?.data?.length === 0 && query !== '';

  const comboBoxChange = async (code: string) => {
    if (!isGameNotFound) {
      setSelectedItem(code);
      // const { gameUid } = offerPromosData!.data!.find(
      //   (promo) => promo.code === code
      // )!;
      navigate(ROUTER_PATH.PROMO_EDIT.replace(ROUTE_PARAM.UID, code));
    }
  };
        ...
        <div className='relative'>
          <div className='relative w-full cursor-default overflow-clip rounded-lg flex flex-nowrap gap-1 items-center bg-brand-black-80 px-3'>
            ...
            <Combobox.Input
              ...
              onChange={onChange}
            />
            <Combobox.Button className='flex items-center' onClick={async () => searchOffers(' ')}>
              {({ open }) => (
                <BiChevronDown
          ...
          >
            <Combobox.Options className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-brand-black-80 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
              {isGameNotFound ? (
                ...
                offerPromosData?.data?.map((offer) => (
                  <Combobox.Option
                    key={offer._id}
                    className={({ active }) =>
