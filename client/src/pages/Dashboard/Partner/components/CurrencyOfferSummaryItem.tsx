...
import { GameCurrencyOfferStatus } from '../../../CreateGameCurrencyOffer/Main';

export type GameCurrencyOffer = {
  id: string;
  currencyUid: string;
  status: GameCurrencyOfferStatus;
};
type Props = {
  payload: GameCurrencyOffer;
};

export const CurrencyOfferSummaryItem = ({ payload }: Props) => {
        ...
        <div className='flex flex-wrap justify-between gap-4'>
          <div className='flex flex-col gap-3'>
            ...
            <h3 className='first-letter:uppercase font-semibold font-tti-demi-bold text-[clamp(1.05rem,2vw,1.5rem)] leading-none'>
              ...
              <span className='text-brand-primary-color-1 capitalize'>{status}</span>
            </h3>
          ...
          <GradientBordered className='rounded-[.25rem] before:rounded-[.25rem] before:transition-all before:bg-gradient-bordered-light overflow-clip inline-flex w-auto h-auto self-start'>
            <Link
              to={ROUTER_PATH.GAME_CURRENCIES_OFFER_EDIT.replace(ROUTE_PARAM.UID, id)}
